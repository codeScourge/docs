...
This token is saved locally on the server (in the session), as well as in the cookies of the user. 
Once the user switches the page, this cookie is read by the server, matched with a token from the session,
...
about the user like his email. Why do we do this? Well, originally this was a way for app-developers to
perform actions on behalf of a user, learn more in [this 5 minute video](https://www.youtube.com/watch?v=KT8ybowdyr0).
However, another emergent benefit is the user experience: it is much easier to have one Google account which
you use to login everywhere else instead of a separate account.


How do we do this? First we need to create a new application at an OAuth provider like in our case GitHub and 
...
For practicality we will use the ``auth_lib`` library. Open ``app.py`` and scroll down to:
```python app.py
# --- OAuth ---
...
Get them from your GitHub application and add them to the ``.env`` file.

Now let's look at the routes managing the authentication. If a user tries to access a route, which he is not authorized to 
use, or manually clicks the sign-up button, he will be redirecte to "/authenticate".
...
From here he is redirected to GitHub using the ``authorize_redirect`` function. To it, we pass a callback URL, which
consists of our host, the "/callback" route and a "next" query parameter which signifies where the user was planning to
...
As you might imagine, once the user has logged in, he is redirected to our "/callback" route. Note, that besides the "next"
query parameter we provided, GitHub also added a "code" one. This is a token which, together with the client secret we can
exchange for an access token. This access token is like an API key and allows us to query data about the user like his email.
...
For that we will call the ``add_user`` function which will add this user with his email to our Database. Continue
reading the next page to learn how