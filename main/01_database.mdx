...
or set them as environment variables, which we are going to do for deployment and security reasons.

To do that, follow [this video](https://www.youtube.com/watch?v=fwtmTMf53Ek) to create a pair of access keys. 
I recommend you to use administrator access while developing and switch to the minimum
rights when deploying to production, so that in case of a leak, the rest of your ressources will be intact.
Set ``AWS_ACCESS_KEY_ID`` and ``AWS_SECRET_ACCESS_KEY_ID`` in your ``.env`` file.
